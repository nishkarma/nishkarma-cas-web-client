 ...
 * @since 3.1
 */
public class NishkarmaCas20ProxyReceivingTicketValidationFilter extends
		NishkarmaAbstractTicketValidationFilter {

	private static final String[] RESERVED_INIT_PARAMS = new String[] {
			ARTIFACT_PARAMETER_NAME.getName(), SERVER_NAME.getName(),
			SERVICE.getName(), RENEW.getName(),
			LOGOUT_PARAMETER_NAME.getName(),
			ARTIFACT_PARAMETER_OVER_POST.getName(),
			EAGERLY_CREATE_SESSIONS.getName(), ENCODE_SERVICE_URL.getName(),
			SSL_CONFIG_FILE.getName(), ROLE_ATTRIBUTE.getName(),
			IGNORE_CASE.getName(), CAS_SERVER_LOGIN_URL.getName(),
			GATEWAY.getName(),
			AUTHENTICATION_REDIRECT_STRATEGY_CLASS.getName(),
			GATEWAY_STORAGE_CLASS.getName(), CAS_SERVER_URL_PREFIX.getName(),
			ENCODING.getName(), TOLERANCE.getName(), IGNORE_PATTERN.getName(),
			IGNORE_URL_PATTERN_TYPE.getName(), HOSTNAME_VERIFIER.getName(),
			HOSTNAME_VERIFIER_CONFIG.getName(),
			EXCEPTION_ON_VALIDATION_FAILURE.getName(),
			REDIRECT_AFTER_VALIDATION.getName(), USE_SESSION.getName(),
			SECRET_KEY.getName(), CIPHER_ALGORITHM.getName(),
			PROXY_RECEPTOR_URL.getName(),
			PROXY_GRANTING_TICKET_STORAGE_CLASS.getName(),
			MILLIS_BETWEEN_CLEAN_UPS.getName(), ACCEPT_ANY_PROXY.getName(),
			ALLOWED_PROXY_CHAINS.getName(), TICKET_VALIDATOR_CLASS.getName(),
			PROXY_CALLBACK_URL.getName(),
			FRONT_LOGOUT_PARAMETER_NAME.getName(),
	...
	protected Class<? extends NishkarmaCas20ServiceTicketValidator> defaultServiceTicketValidatorClass;

	protected Class<? extends NishkarmaCas20ProxyTicketValidator> defaultProxyTicketValidatorClass;
	...
	public NishkarmaCas20ProxyReceivingTicketValidationFilter() {
		this(Protocol.CAS2);
		this.defaultServiceTicketValidatorClass = NishkarmaCas20ServiceTicketValidator.class;
		this.defaultProxyTicketValidatorClass = NishkarmaCas20ProxyTicketValidator.class;
	}

	protected NishkarmaCas20ProxyReceivingTicketValidationFilter(
	...
	private <T> T createNewTicketValidator(
			final Class<? extends NishkarmaCas20ServiceTicketValidator> ticketValidatorClass,
		...
		return (T) ReflectUtils.newInstance(ticketValidatorClass,
				casServerUrlPrefix);
	}
	...
	protected final TicketValidator getTicketValidator(
		...
		final Class<? extends NishkarmaCas20ServiceTicketValidator> ticketValidatorClass = getClass(new ConfigurationKey<Class<? extends NishkarmaCas20ServiceTicketValidator>>(
				"ticketValidatorClass", null));

		final NishkarmaCas20ServiceTicketValidator validator;

		if (allowAnyProxy || CommonUtils.isNotBlank(allowedProxyChains)) {
			final NishkarmaCas20ProxyTicketValidator v = createNewTicketValidator(
			...
			validator = createNewTicketValidator(ticketValidatorClass,
					casServerUrlPrefix, this.defaultServiceTicketValidatorClass);
		}
		validator
				.setProxyCallbackUrl(getString(ConfigurationKeys.PROXY_CALLBACK_URL));
		validator
		...
		validator.setProxyRetriever(new Cas20ProxyRetriever(casServerUrlPrefix,
				getString(ConfigurationKeys.ENCODING), factory));
